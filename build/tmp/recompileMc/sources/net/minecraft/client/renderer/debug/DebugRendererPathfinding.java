        ...
        if (this.pathMap.size() != 0)
        {
            ...
            for (Integer integer1 : this.pathMap.keySet())
            {
                ...
                for (PathPoint pathpoint3 : path1.getClosedSet())
                {
                    ...
                    {
                        DebugRenderer.renderDebugText(String.format("%s", new Object[] {pathpoint3.nodeType}), (double)pathpoint3.x + 0.5D, (double)pathpoint3.y + 0.75D, (double)pathpoint3.z + 0.5D, partialTicks, -65536);
                        DebugRenderer.renderDebugText(String.format("%.2f", new Object[] {Float.valueOf(pathpoint3.costMalus)}), (double)pathpoint3.x + 0.5D, (double)pathpoint3.y + 0.25D, (double)pathpoint3.z + 0.5D, partialTicks, -65536);
                    }
                }
                ...
                for (PathPoint pathpoint4 : path1.getOpenSet())
                {
                    ...
                    {
                        DebugRenderer.renderDebugText(String.format("%s", new Object[] {pathpoint4.nodeType}), (double)pathpoint4.x + 0.5D, (double)pathpoint4.y + 0.75D, (double)pathpoint4.z + 0.5D, partialTicks, -16776961);
                        DebugRenderer.renderDebugText(String.format("%.2f", new Object[] {Float.valueOf(pathpoint4.costMalus)}), (double)pathpoint4.x + 0.5D, (double)pathpoint4.y + 0.25D, (double)pathpoint4.z + 0.5D, partialTicks, -16776961);
                    }
                }
                ...
                for (int k = 0; k < path1.getCurrentPathLength(); ++k)
                {
                    ...
                    {
                        DebugRenderer.renderDebugText(String.format("%s", new Object[] {pathpoint2.nodeType}), (double)pathpoint2.x + 0.5D, (double)pathpoint2.y + 0.75D, (double)pathpoint2.z + 0.5D, partialTicks, -1);
                        DebugRenderer.renderDebugText(String.format("%.2f", new Object[] {Float.valueOf(pathpoint2.costMalus)}), (double)pathpoint2.x + 0.5D, (double)pathpoint2.y + 0.25D, (double)pathpoint2.z + 0.5D, partialTicks, -1);
                    }
                }
            }
    ...
    public void renderPathLine(float finishTimeNano, Path p_190067_2_)
    {
        ...
        for (int i = 0; i < p_190067_2_.getCurrentPathLength(); ++i)
        {
            ...
            if (this.addDistanceToPlayer(pathpoint) <= 40.0F)
            {
                ...
                int l = j >> 8 & 255;
                int i1 = j & 255;
                vertexbuffer.pos((double)pathpoint.x - this.xo + 0.5D, (double)pathpoint.y - this.yo + 0.5D, (double)pathpoint.z - this.zo + 0.5D).color(k, l, i1, 255).endVertex();
            }
        }
